function updategame(output,mapsize,radius,opentab){
	...
	if (output.maprefresh) {
		$.post("minimap.php",{dud:0},function(data,status){
	...
	if (opentab=="items"){
		...
		for (item=0;item<14;item++){
			if (typeof output.stats.inventory[item]!='undefined'){
				...
				$("#inventory [value="+item+"]").attr("src","getimage.php?imageof="+itemtype+"&tilesize=32");
				newimg = document.getElementById(itemtype);
				ctx.drawImage(newimg,136+(item%7)*36,378+Math.floor(item/7)*50,36,36);
			} else {
				$("#inventory [value="+item+"]").attr("src","getimage.php?imageof=none&tilesize=32");
			}
		}
		for (item=0;item<7;item++){
			if (output.stats.equipment[item]){
				var itemtype=output.stats.equipment[item].type;
				$("#equipment [value="+item+"]").attr("src","getimage.php?imageof="+itemtype+"&tilesize=36");
				newimg = document.getElementById(itemtype);
				ctx.drawImage(newimg,136+item*36,326,36,36);
			} else {
				$("#equipment [value="+item+"]").attr("src","getimage.php?imageof=none&tilesize=36");
			}
		}
		if (typeof output.stats.onground!='undefined') for (item=0;item<7;item++){
			if (typeof output.stats.onground[item]!='undefined'){
				...
				$("#ground [value="+item+"]").attr("src","getimage.php?imageof="+itemtype+"&tilesize=32");
				newimg = document.getElementById(itemtype);
				ctx.drawImage(newimg,136+item*36,472,36,36);
			} else {
				$("#ground [value="+item+"]").attr("src","getimage.php?imageof=none&tilesize=32");
			}
		}
	}
	...
	for (spell=0;spell<7;spell++){
		if (typeof output.stats.repetoire[spell]!='undefined'){
			...
			$("#spells [value="+spell+"]").attr("src","getimage.php?imageof="+spelltype+"&tilesize=32");
		} else {
			$("#spells [value="+spell+"]").attr("src","getimage.php?imageof=none&tilesize=32");
		}
	}
...
function animationcycle(output,lastoutput,ctx,mapsize,radius,opentab){
	...
	function animate(animations,done){
		if (animations[done].type=="bleed"){
			...
			output=updategame(output,mapsize,radius);
			timer=0;
			playerdeath();
		}
		done++;
